...
let micropub_query req =
  ...
  else if q = "source" then
    if url = "" then raise (Error_invalid_request "missing url")
    else
      let uid, slug = get_uid_and_slug (url |> Uri.of_string |> Uri.path) in
      Entry.get uid
      >>= fun some_stored ->
      match some_stored with
      | Some stored -> `Json stored |> respond' ~headers
      | None ->
...
let micropub_update_delete current_data jdata =
  if not Ezjsonm.(mem jdata ["delete"]) then Ezjsonm.(get_dict current_data)
  else
    ...
    let new_props =
      ...
      | _ -> failwith "invalid delete payload"
    in
...
let micropub_update_add current_data jdata =
  if not Ezjsonm.(mem jdata ["add"]) then Ezjsonm.(get_dict current_data)
  else
    let props = Ezjsonm.(get_dict (find current_data ["properties"])) in
...
let micropub_update_replace current_data jdata =
  if not Ezjsonm.(mem jdata ["replace"]) then Ezjsonm.(get_dict current_data)
  else
    let props = Ezjsonm.(get_dict (find current_data ["properties"])) in
...
let micropub_update url jdata =
  if url = "" then raise (Error_invalid_request "missing url")
    ...
    | Some stored ->
        ...
        >>= fun _ ->
        let headers =
          Cohttp.Header.init () |> add_header "Location" (build_url uid slug)
        in
...
let handle_json_create jdata scopes =
  ...
  if action = "delete" then
    ...
    if List.mem action scopes then micropub_delete url
    else raise (Error_insufficient_scope "missing \"delete\" scope")
  else if action = "update" then
    (* Ensure the delete scope is there *)
    if List.mem action scopes then micropub_update url jdata
  ...
  then raise (Error_insufficient_scope "missing \"create\" scope")
    ...
    let slug =
      jform_field jdata ["properties"; "mp-slug"] (slugify entry_name)
    in
