...
let parse url soup =
  let title =
    try soup $ "title" |> fun node -> node |> R.leaf_text |> String.trim with
    | _ ->
        url
  in
...
let handle_relative_url url url_or_path =
  if url_or_path = ""
  then ""
  else if String.sub url_or_path 0 1 = "/"
  then Uri.(to_string (with_path (of_string url) url_or_path))
  else url_or_path
...
let send_webmentions source body =
  ...
  Lwt_list.map_s
    (fun target ->
      ...
      match wu with
      | Some webmention_url ->
          send_webmention source target webmention_url
      | None ->
          Lwt.return false)
    targets
...
let iter uid map =
  ...
  Lwt_list.map_s
    (fun (s, c) ->
      ...
      >|= fun stored -> stored |> Ezjsonm.from_string |> add_last_updated_pretty)
    keys
...
let save_webmention uid mention js =
  ...
  Lwt_list.map_s
    (fun (s, c) ->
      Store.get t [ "webmentions"; uid; s ]
      >|= fun stored ->
      if Ezjsonm.(get_string (find (from_string stored) [ "url" ])) = u
      then (true, s)
      else (false, s))
    keys
  >>= fun dat ->
...
let bad_url u =
  ...
  if scheme = "" || not (scheme = "http" || scheme = "https")
  then true
  else
