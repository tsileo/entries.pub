...
let verify_incoming_webmention url target =
  ...
  in
  List.mem target links
  |> Lwt.return

let wrap_some v = Some v

let discover_webmention_html soup =
  try
    soup $ "link[rel=webmention]" |> R.attribute "href" |> wrap_some |> Lwt.return
  with Failure _ -> None |> Lwt.return
...
let discover_webmention_header resp =
  let links = 
    ...
    |> List.filter (fun ({ context; arc; target }) -> List.exists (fun x -> x = rel_webmention) arc.relation) in
            (* List.hd arc.relation = rel_webmention) in *)
