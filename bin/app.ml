...
let logger =
  let filter handler req =
    ...
    Logs.info (fun m ->
        m "[%s] - \"%s %s HTTP/1.1\" %d \"%s\" \"%s\" %f" date meth uri code
          referer ua
...
let get_feed_json =
  App.get "/feed.json" (fun req ->
      Entriespub.Entry.iter (fun item ->
          ...
          if is_page then `O [("is_page", `Bool true)]
          else
            `O
      ...
      >>= Entriespub.Entry.discard_pages_and_drafts
      >>= fun items ->
      let headers =
        Cohttp.Header.init ()
...
let get_entry =
  App.get "/:uid/:slug" (fun req ->
      ...
      Entriespub.Entry.get uid
      >>= fun some_stored ->
      match some_stored with
      | Some stored ->
          (* Fetch associated Webmentions *)
          Entriespub.Webmention.iter uid (fun x -> x)
          >>= fun webmentions ->
          (* Render the entry *)
      ...
      | None ->
          ...
          let out = Mustache.render html_tpl dat in
          let headers =
            Cohttp.Header.init () |> add_micropub_header base_url
          in
      ...
      if auth = false then
        ...
        try%lwt
          if content_type = "application/json" then
            ...
            req |> App.urlencoded_pairs_of_body
            >>= fun dat -> Entriespub.Micropub.handle_form_create dat scopes
        with
...
let () =
  ...
  in
  let app =
    App.empty |> head_feed_json |> get_feed_json |> head_feed_atom
    |> get_feed_atom |> head_index |> get_index |> head_entry |> get_entry
    |> post_webmention |> get_micropub |> post_micropub |> middleware static
    |> middleware logger |> App.port 7888 |> App.run_command'
  in
  match app with
  | `Ok app ->
      let s =
